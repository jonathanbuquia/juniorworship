...
function useDraggableSwimmer({
  ...
  const [releasedStart, setReleasedStart] = useState(null)
  ...
  useEffect(() => {
    ...
    const handlePointerUp = () => {
      setDragState((previous) => {
        ...
        setReleasedStart(previous.position)
  ...
  }, [dragState, height, maxYInset, minY, tankRef, width])
  ...
  return {
    ...
    restartKey,
    startDragging,
    startOverride: releasedStart,
  }
}

function NaturalFish({ fish, tankRef, tankSize }) {
...
function CuteOctopus({ tankRef, tankSize }) {
  const draggable = useDraggableSwimmer({
    tankRef,
...
function CuteJellyfish({ tankRef, tankSize }) {
  ...
  const showMessage = () => {
    ...
    setMessage(jellyfishMessages[Math.floor(Math.random() * jellyfishMessages.length)])
    setIsTalking(true)
  ...
  return (
    ...
    >
      {isTalking && (
        <div className="jellyfish-bubble" aria-hidden="true">
          {message}
        </div>
      )}
...
function CuteCrab({ tankRef, tankSize }) {
...
function CutePufferfish({ tankRef, tankSize }) {
  const draggable = useDraggableSwimmer({
    tankRef,
...
function CuteStingray({ tankRef, tankSize }) {
  const draggable = useDraggableSwimmer({
    tankRef,
...
function CuteTurtle({ tankRef, tankSize }) {
  const draggable = useDraggableSwimmer({
    tankRef,
...
function App() {
  ...
  const [coralPositions, setCoralPositions] = useState({})
  const [dragState, setDragState] = useState(null)
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      setCoralPositions((previous) => {
        const next = { ...previous }

        coralDecorations.forEach((coral) => {
          const current = previous[coral.id]
      ...
      <section className="aquarium-scene" aria-label="Pixel aquarium">
        ...
        <div className="tank-wrap">
          ...
          <div className="tank" ref={tankRef}>
            ...
            {tankSize.width > 0 && <CuteTurtle tankRef={tankRef} tankSize={tankSize} />}
            {tankSize.width > 0 && <CuteStingray tankRef={tankRef} tankSize={tankSize} />}
            {tankSize.width > 0 && <CutePufferfish tankRef={tankRef} tankSize={tankSize} />}
            {tankSize.width > 0 && <CuteCrab tankRef={tankRef} tankSize={tankSize} />}
            {tankSize.width > 0 && <CuteJellyfish tankRef={tankRef} tankSize={tankSize} />}
            {tankSize.width > 0 && <CuteOctopus tankRef={tankRef} tankSize={tankSize} />}

            {tankSize.width > 0 &&
              school.map((fish) => (
                <NaturalFish key={fish.id} fish={fish} tankRef={tankRef} tankSize={tankSize} />
              ))}
          </div>
        </div>
      </section>
