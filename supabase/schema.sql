...
alter table public.shop_items
  add column if not exists category text not null default 'fish';
...
as $function$
  ...
  validated_purchase as (
    ...
    where locked_profile.gold >= selected_item.price
      and not (
        selected_item.category = 'events'
        and exists (
          select 1
          from public.inventory
          where inventory.user_id = p_user_id
            and inventory.item_id = selected_item.id
...
insert into public.shop_items (slug, name, description, price, accent_color, category)
values
  ('starter-food', 'Starter Food', 'A simple pellet pack that keeps the fish tank busy.', 25, '#80d7ff', 'others'),
  ('coral-cave', 'Coral Cave', 'A bright coral shelter for the lower side of the tank.', 70, '#ff8f73', 'aquarium'),
  ('moon-jelly', 'Moon Jelly', 'Unlock after owning 2 each of the three common starter fish.', 1500, '#bdb2ff', 'events')
